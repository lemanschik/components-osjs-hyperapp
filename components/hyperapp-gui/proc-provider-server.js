...
class ProcServiceProvider {
  ...
  constructor(core, options = {}) {
    this.core = core;
    this.options = {groups: ['admin'], ...options};
    this.processes = [];
  ...
  init() {
    ...
    const spawnCallback = methodName => ({body, session}, res) => {
      const {name, cmd, args} = body;
    ...
    route('/proc/kill', ({body}, res) => { // FIXME: Make a WS signal
      const {name} = body;
  ...
  broadcastMessage(username, name, type, ...args) {
    this.core.broadcast('osjs/proc-provider:stdout', [{name, type}, ...args], ({_osjs_client}) => _osjs_client && _osjs_client.username === username);
  }
  ...
  checkTimeouts() {
    ...
    const timedOut = this.processes
      .filter(({lastTime}) => (now - lastTime) >= TIMEOUT_PING)
      .map(({name}) => name);
